...
## `--as-app`: test things in the app's exact sandbox

`--as-app` runs any command inside the same sandbox + env a request to the app would get — the way to test how something *behaves under production isolation*, interactively or not.
...
See what it runs under the hood:

```sh
./reverse-bin-detector --as-app ~/apps/demo -- /bin/true
```
